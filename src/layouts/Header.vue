        ...
        <nav class="container mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <div class="hidden md:flex space-x-8 items-center">
                <a href="#about" class="hover:text-sky-400 transition-colors duration-300"
                    :class="{ 'text-sky-400': locationHash === '#about' }">{{ t('navigation.about') }}</a>
                ...
                <a href="#experience" class="hover:text-sky-400 transition-colors duration-300"
                    :class="{ 'text-sky-400': locationHash === '#experience' }">{{ t('resume.experience') }}</a>
                <a href="#projects" class="hover:text-sky-400 transition-colors duration-300"
                    :class="{ 'text-sky-400': locationHash === '#projects' }">{{ t('navigation.projects') }}</a>
                <a href="#contact"
                    class="bg-sky-500 hover:bg-sky-600 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-300"
                    :class="{ 'bg-sky-600 text-white': locationHash === '#contact' }">{{ t('navigation.contact') }}</a>
            ...
            <div v-show="isMenuOpen" class="md:hidden" id="mobile-menu">
                <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3 flex flex-col items-center">
                    <a href="#about" @click="toggleMenu"
                        class="block px-3 py-2 rounded-md text-base font-medium text-slate-300 hover:text-sky-400 hover:bg-slate-800 w-full text-center">{{ t('navigation.about') }}</a>
                    ...
                    <a href="#experience" @click="toggleMenu"
                        class="block px-3 py-2 rounded-md text-base font-medium text-slate-300 hover:text-sky-400 hover:bg-slate-800 w-full text-center">{{ t('resume.experience') }}</a>
                    <a href="#projects" @click="toggleMenu"
                        class="block px-3 py-2 rounded-md text-base font-medium text-slate-300 hover:text-sky-400 hover:bg-slate-800 w-full text-center">{{ t('navigation.projects') }}</a>
                    <a href="#contact" @click="toggleMenu"
                        class="block px-3 py-2 rounded-md text-base font-medium text-slate-300 hover:text-sky-400 hover:bg-slate-800 w-full text-center">{{ t('navigation.contact') }}</a>
                    <button @click="toggleLocales"
                        class="text-white bg-slate-800 hover:bg-slate-700 focus:ring-4 focus:outline-none focus:ring-slate-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-slate-600 dark:hover:bg-slate-700 dark:focus:ring-slate-800">
                        {{ locale === 'en' ? 'ID' : 'EN' }}
                    </button>
